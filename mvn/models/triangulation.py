    ...
    def forward(self, images, proj_matricies, batch):
        ...
        for view_i in range(n_views):
            for batch_i in range(batch_size):
                ...
                print('camera ', new_cameras[view_i][batch_i].params)
        ...
        for batch_i in range(batch_size):
            ...
            if self.use_gt_pelvis:
                ...
                print('pr ed_keypoints_3d ', keypoints_3d)
...
class ModifiedVolTriNet(nn.Module):
    def __init__(self,  device='cuda:0'):
        super().__init__()

        self.num_joints = 17  # config.model.backbone.num_joints
        self.volume_aggregation_method = "softmax" # config.model.volume_aggregation_method

        # volume
        self.volume_softmax = True # config.model.volume_softmax
        self.volume_multiplier = 1000.0 # config.model.volume_multiplier
        self.volume_size = 64 # config.model.volume_size

        self.cuboid_side = 2500.0 # config.model.cuboid_side

        self.kind = "mpii" # config.model.kind
        self.use_gt_pelvis = False # config.model.use_gt_pelvis

        # heatmap
        self.heatmap_softmax = True # config.model.heatmap_softmax
        self.heatmap_multiplier = 100.0 # config.model.heatmap_multiplier

        # transfer
        self.transfer_cmu_to_human36m = False # config.model.transfer_cmu_to_human36m if hasattr(config.model, "transfer_cmu_to_human36m") else False
        ...
        self.backbone = pose_resnet.get_pose_net(device=device)
    ...
    def forward(self, images, cameras, pred_keypoints_3d):
        device = images.device
        batch_size, n_views = images.shape[:2]
